...
(define (get-coercion from-type to-type)
  (table-get (list 'coerce from-type to-type)))
(define (put-coercion from-type to-type value)
  (table-put (list 'coerce from-type to-type) value))
...
(define (apply-generic op . args)
  ...
  (let* ((type-tags (map type-tag args))
    ...
    (define (try-coercion tags)
      (if (null? tags)
          (error "No method for these types"
                 (list op type-tags))
          (let ((coerce-funcs (map (lambda (t)
                                     (if (eq? t (car tags))
                                         identity
                                         (get-coercion t (car tags))))
                                   type-tags)))
            (if (every identity coerce-funcs)
                (apply apply-generic
                       (cons op (map (lambda (f t)
                                       (f t))
                                     coerce-funcs args)))
    ...
    (if proc
        ...
        (let ((type-tags-set (remove-duplicates type-tags)))
          (if (= 1 (length type-tags-set))
              (error "No method for these types"
                     (list op type-tags))
              (try-coercion type-tags-set))))))
